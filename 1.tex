...
A coleta de dados é em sua organização moderna praticamente inescapável, porém, é pouco enunciada em sua totalidade.
...
Apesar de ser feita às escondidas, isto é, sem a ciência do usuário, essa coleta é facilmente rastreável, de modo que mesmo com pouca experiência um indivíduo pode analisar os dados que transitam por seus dispositivos.

Toda essa burocracia trata da passagem de dados produzidos pelo usuário, em seu computador, para o servidor onde é hospedado o serviço em questão.
...
Quando se entra num site, o navegador não se comunica apenas com o servidor, mas também com diversas outras fontes terceiras para múltiplos propósitos diferentes, que vão desde obter bibliotecas de código\footnote{Uma biblioteca é um conjunto de código pronto feito para ser integrado em outros códigos, implementando utilidades genéricas e de uso comum.}, até carregar imagens, informações, texto, vídeos, entre outros.
Essas operações, que por vezes passam de centenas para um único carregamento, ocorrem escondidas: o que o usuário entende como uma única requisição (entrar no site) é um processo que provoca dezenas de outras, sem que ele perceba.

Muitas vezes, para ter acesso a serviços da Google (e de outros provedores) os sites precisam carregar pedaços de código criados por esse provedores, os chamados Kits de Desenvolvimento de Software (SDKs, do inglês \textit{software development kit}), que tomam controle parcial do tráfego da página.
...
Em troca, obtém-se dinheiro pelos anúncios e dados das interações com a página, a fim de saber quem são os usuários e como estão usando o serviço.
É comum que SDKs ou outras bibliotecas sejam incluídas propositalmente para rastrear o usuário, pois isso, para o desenvolvedor do site, é uma vantagem: captar tantas informações sobre o usuário permite uma otimização da página.
...
Os anunciantes dão seus lances baseados em pacotes de dados do usuário-alvo que o site (chamado de \textit{supply side platform} ou SSP, a plataforma de oferta) distribui.
...
Pode ocorrer que um dos participantes do leilão nem sequer tenha interesse em vender um anúncio, sempre dando lances inviáveis e se aproveitando disso para colher dados.
...
Está se tornando cada vez mais comum o uso de perfis de direção por seguros de carros, gerados a partir de sensores de velocidade e/ou posição implantados do carro\cite{mckinsey_monetizing_2016, hill_automakers_2024}.
...
Por meio da coleta de dados, o seguro consegue ter uma imagem compreensiva do motorista, e recalibrar-se nos moldes dessa imagem, ajustando seu preço para punir os motoristas desmiolados e descuidados que andam excessivamente rápido, e beneficiar os motoristas que dirigem calmamente.
A diferença desse modelo para os seguros que não vigiam seus motoristas é que, nos primeiros, pode-se otimizar o serviço de um modo mais granular, a fim de maximizar a margem de lucro e minimizar a chance do seguro ter déficit financeiro com um de seus clientes.
...
\section{Vigilância e política}\label{sec:1:politica}
